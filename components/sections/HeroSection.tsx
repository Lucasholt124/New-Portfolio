            ...
            <div className="order-1 lg:order-2 animate-fade-in-up animation-delay-200">
              {profile.profileImage && (
                <div className="relative group max-w-[320px] sm:max-w-md md:max-w-lg mx-auto lg:max-w-none">
                    ...
                    <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/70 via-black/40 to-transparent p-4 sm:p-5 lg:p-6 translate-y-full group-hover:translate-y-0 transition-transform duration-500 ease-out pointer-events-none">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-white/90 text-xs sm:text-sm font-medium">
                          {profile.availability || "Disponível para projetos"}
                        </span>
                      </div>
                  ...
                  <div className="flex justify-center gap-2.5 sm:gap-3 mt-6 sm:hidden">
                    {stats.map((stat, i) => {
                  ...
                  <div className="hidden sm:block">
                    {stats.map((stat, i) => {
                      const StatIcon = stat.icon;
                      const positions = [
                        "-top-4 -left-6 lg:-left-12",
                        "-bottom-4 -right-6 lg:-right-12",
                        "-bottom-4 -left-4 lg:-left-10",
                      ];

                      return (
                        <div
                          key={i}
                          className={`absolute ${positions[i]} z-30 animate-float-card`}
                          style={{ animationDelay: `${i * 2}s` }}
                        >
                          <div className="flex items-center gap-3 px-4 py-3 rounded-xl border border-border/50 bg-card/90 backdrop-blur-md shadow-lg hover:shadow-xl hover:scale-105 transition-all duration-300">
